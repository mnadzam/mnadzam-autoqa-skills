...
- If matched_pattern is not null, it should be a plausible pattern name
...
def main() -> int:
    ...
    if pattern is not None and not re.match(r"^[a-z][a-z0-9_]*$", pattern):
        print(
            f"Semantic error: matched_pattern '{pattern}' does not look like "
            "a valid pattern name (expected lowercase_with_underscores)",
            file=sys.stderr,
        )
        return 1
